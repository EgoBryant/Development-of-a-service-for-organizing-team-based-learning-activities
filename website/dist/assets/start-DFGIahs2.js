(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))r(t);new MutationObserver(t=>{for(const a of t)if(a.type==="childList")for(const f of a.addedNodes)f.tagName==="LINK"&&f.rel==="modulepreload"&&r(f)}).observe(document,{childList:!0,subtree:!0});function s(t){const a={};return t.integrity&&(a.integrity=t.integrity),t.referrerPolicy&&(a.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?a.credentials="include":t.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function r(t){if(t.ep)return;t.ep=!0;const a=s(t);fetch(t.href,a)}})();const e={view:"home",signIn:{email:"",password:""},signUp:{email:"",code:"",codeSent:!1,password:"",passwordConfirm:""},recovery:{email:"",password:""}},S=document.getElementById("homeScreen"),v=document.getElementById("appScreen"),I=document.getElementById("authLayout"),l=document.getElementById("formContent"),d=document.getElementById("promoButton");function p(n){return n instanceof HTMLElement}function g(n){return n instanceof HTMLInputElement}function w(n){return n instanceof HTMLButtonElement}function y(n){return n instanceof HTMLFormElement}function h(n,o){const s=document.querySelector(n);if(!o(s))throw new Error(`Required element not found: ${n}`);return s}function i(n){return g(n)?n.value:""}function c(n){e.view=n,M()}function m(n){return n.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function M(){if(!p(S)||!p(v)||!p(I)||!p(l)||!w(d))return;const n=e.view==="home";S.classList.add("screen-active"),v.classList.toggle("screen-active",!n),v.setAttribute("aria-hidden",String(n)),document.body.classList.toggle("modal-open",!n),n||x()}function x(){if(!p(I)||!p(l)||!w(d))return;const n=e.view,o=n==="sign-up"||n==="sign-up-confirm";if(I.classList.toggle("layout-reverse",o),n==="sign-in"){d.textContent="РЕГИСТРАЦИЯ",d.onclick=()=>c("sign-up"),l.innerHTML=`
        ...
        `;const s=h("#signInForm",y);s.addEventListener("submit",r=>{r.preventDefault(),e.signIn.email=i(s.elements.namedItem("email")),e.signIn.password=i(s.elements.namedItem("password")),c("sign-in-confirm")})}if(n==="sign-up"){d.textContent="ВХОД",d.onclick=()=>c("sign-in"),l.innerHTML=`
            <form id="signUpForm" class="auth-form">
                ...
                <button class="primary-button form-button" id="signUpSubmitButton" type="submit" ${b()?"":"disabled"}>ПРИСОЕДИНИТЬСЯ</button>
            </form>
        `;const s=h("#signUpForm",y);N(s),s.addEventListener("submit",r=>{r.preventDefault(),b()&&(e.signUp.email=i(s.elements.namedItem("email")),e.signUp.code=i(s.elements.namedItem("code")),e.signUp.password=i(s.elements.namedItem("password")),e.signUp.passwordConfirm=i(s.elements.namedItem("passwordConfirm")),c("sign-up-confirm"))})}if(n==="recovery"){d.textContent="РЕГИСТРАЦИЯ",d.onclick=()=>c("sign-up"),l.innerHTML=`
        ...
        `;const s=h("#recoveryForm",y);s.addEventListener("submit",r=>{r.preventDefault(),e.recovery.email=i(s.elements.namedItem("email")),e.recovery.password=i(s.elements.namedItem("password")),e.signIn.email=e.recovery.email,e.signIn.password=e.recovery.password,c("sign-in")})}n==="sign-in-confirm"&&(d.textContent="РЕГИСТРАЦИЯ",d.onclick=()=>c("sign-up"),l.innerHTML=`
            <div class="auth-form">
                <h1>ПОДТВЕРЖДЕНИЕ</h1>
        ...
        `),l.querySelectorAll("[data-view]").forEach(s=>{s.addEventListener("click",()=>{if(n==="sign-in"){const t=document.getElementById("signInForm");t instanceof HTMLFormElement&&(e.signIn.email=i(t.elements.namedItem("email")),e.signIn.password=i(t.elements.namedItem("password")))}if(n==="sign-up"){const t=document.getElementById("signUpForm");t instanceof HTMLFormElement&&(e.signUp.email=i(t.elements.namedItem("email")),e.signUp.code=i(t.elements.namedItem("code")),e.signUp.password=i(t.elements.namedItem("password")),e.signUp.passwordConfirm=i(t.elements.namedItem("passwordConfirm")))}if(n==="recovery"){const t=document.getElementById("recoveryForm");t instanceof HTMLFormElement&&(e.recovery.email=i(t.elements.namedItem("email")),e.recovery.password=i(t.elements.namedItem("password")))}const r=s.dataset.view;r&&c(r)})})}function b(){return!!(e.signUp.email.trim()&&e.signUp.codeSent&&e.signUp.code.trim()&&e.signUp.password&&e.signUp.passwordConfirm&&e.signUp.password===e.signUp.passwordConfirm)}function F(){return!!(e.signUp.password&&e.signUp.passwordConfirm&&e.signUp.password!==e.signUp.passwordConfirm)}function N(n){const o=n.elements.namedItem("email"),s=n.elements.namedItem("code"),r=n.elements.namedItem("password"),t=n.elements.namedItem("passwordConfirm");if(!g(o)||!g(r))return;const a=document.getElementById("sendCodeButton"),f=document.getElementById("repeatCodeButton"),U=document.getElementById("codeRow"),L=document.getElementById("passwordMismatchMessage"),E=document.getElementById("signUpSubmitButton");if(!w(a)||!p(U)||!p(L)||!w(E))return;const B=a,C=w(f)?f:null,H=U,T=L,$=E;function u(){B.classList.toggle("hidden",!e.signUp.email.trim()),H.classList.toggle("hidden",!e.signUp.codeSent),g(t)&&t.classList.toggle("hidden",!e.signUp.password),T.classList.toggle("hidden",!F()),$.disabled=!b()}o.addEventListener("input",()=>{const q=e.signUp.email;e.signUp.email=o.value,q!==e.signUp.email&&e.signUp.codeSent&&(e.signUp.codeSent=!1,e.signUp.code="",g(s)&&(s.value="")),u()}),B.addEventListener("click",()=>{e.signUp.email=o.value,e.signUp.email.trim()&&(e.signUp.codeSent=!0,u())}),C&&C.addEventListener("click",()=>{e.signUp.email=o.value,e.signUp.email.trim()&&(e.signUp.codeSent=!0,u())}),g(s)&&s.addEventListener("input",()=>{e.signUp.code=s.value,u()}),r.addEventListener("input",()=>{e.signUp.password=r.value,u()}),g(t)&&t.addEventListener("input",()=>{e.signUp.passwordConfirm=t.value,u()}),u()}document.querySelectorAll("[data-view]").forEach(n=>{n.addEventListener("click",()=>{const o=n.dataset.view;o&&c(o)})});M();
